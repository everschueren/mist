...
preprocess.orderExperiments <- function(y, id_colname) {
    ...
    tmp = strsplit(idnum, "-")
    tmp = matrix(unlist(lapply(tmp, function(x) if (length(x) > 1) {
        c(x[1], x[2])
    } else {
        c(x[1], 0)
    })), ncol = 2, byrow = T)
...
preprocess.findCarryover <- function(x, id_colname, prey_colname, pepcount_colname) {
    ...
    for (i in idx) {
        ...
        be_gone <- which(tmp[, pepcount_colname] > 0 & tmp[, pepcount_colname] < (x[i, pepcount_colname]/2) & 
            prey < length(experiments)/3)
        if (length(be_gone)) {
...
preprocess.createMatrix <- function(y, collapse_file, exclusions_file, remove_file, id_colname, prey_colname, 
    pepcount_colname, mw_colname) {
...
preprocess.main <- function(data_file, keys_file, output_file, filter_data, contaminants_file, rm_co = T, collapse_file, 
    exclusions_file, remove_file, id_colname, prey_colname, pepcount_colname, mw_colname) {
    cat("\tREADING FILES\n")
    ...
    if (rm_co == 1) {
        ...
        if (length(to_remove) > 0) {
            ...
            tmp = merge(df, data.frame(to_remove[, c(id_colname, prey_colname)], here = 1), by = c(id_colname, 
                prey_colname), all.x = TRUE)  #get index of carryover proteins
    ...
    df_mat <- preprocess.createMatrix(df, collapse_file, exclusions_file, remove_file, id_colname, prey_colname, 
        pepcount_colname, mw_colname)  #return a list b/c of space padding
