...
# plotROC = function(prediction_rates){ dim_predictions = round(sqrt(length(unique(prediction_rates$ID))))
...
mist.train.main = function(metrics, training_set, output_file, training_steps = 0.1) {
    ...
    prediction_weights = data.frame(R = prediction_weights[1], A = prediction_weights[2], S = prediction_weights[3], 
        stringsAsFactors = F)
