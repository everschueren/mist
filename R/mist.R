...
mist.getMetrics <- function(x, info) {
    ...
    for (i in unique(info$Bait)) {
        ...
        if (length(bidx) < 2) {
            # if there's only one row for that bait
            y <- sum(y)
        } else {
            y <- rowSums(y)
        }
    ...
    # Specificity ---------------------- Must account for specificity exclusions
...
mist.getSpecificity <- function(abundance, info) {
    ...
    for (i in 1:dim(abundance)[2]) {
        ...
        baits_to_exclude <- strsplit(baits_to_exclude, split = "\\|")
        ...
        if (length(baits_to_include) == 1) {
            specificity[, i] <- 1
        } else if (length(baits_to_exclude) > 0 & sum(rowSums(abundance[, baits_to_include])) > 0) {
            # there are exclusions AND all the rowsums of the data != 0
            specificity[, i] <- specificity[, i]/rowSums(abundance[, baits_to_include])
        } else if (length(baits_to_exclude) == 0 & sum(rowSums(abundance)) > 0) {
            # no exclusions AND all the rowsums of the data != 0
            specificity[, i] <- specificity[, i]/rowSums(abundance)
        } else {
            # rowsums of the data = 0
            specificity[, i] <- 0
        }
...
mist.getSampleOccurences = function(m3d_norm, info) {
    ...
    ip_occurences = aggregate(data_merged[, c("Bait", "Prey", "Ip")], by = as.list(data_merged[, c("Bait", "Prey")]), 
        FUN = function(x) paste(unique(x), collapse = ","))
    ip_occurences[, c("Bait", "Prey", "Ip")]
}

# scores <- cbind(scores, mist_hiv=scores$Repro*0.30853 + scores$Abundance*0.00596 +
...
mist.main <- function(matrix_file, weights = "fixed", w_R = 0.30853, w_A = 0.00596, w_S = 0.68551, training_file, 
    ...
    ## output_file, row.names=FALSE, col.names=TRUE, quote=FALSE, sep='\t' )
...
# file containing values'), make_option(c('-b', '--exclusions_file'), help='exclusions file'),
# make_option(c('-n', '--MAT_name'), help='name of the MAT data file'), make_option(c('-s', '--self_score'),
# help='calculate MIST self score (0/1)') ) parsedArgs = parse_args(OptionParser(option_list = option_list),
# args = commandArgs(trailingOnly=T)) }

## TODO: make the following code into a unit-test config =
...
## mist.main(matrix_file=config$mist$matrix_file, weights=config$mist$weights, w_R=config$mist$reproducibility,
